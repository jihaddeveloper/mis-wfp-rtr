...
//  Description: DI Monthly book checkout service file
...
@Service
@Transactional
public class DIMonthlyBookCheckoutCheckinService {

    @Autowired
    private DIMonthlyBookCheckoutCheckinRepo diMonthlyBookCheckoutCheckinRepo;

    //Create new di monthly-book-checkout
    public DIBookCheckoutCheckin createBookCheckout(DIBookCheckoutCheckin monthlyBookCheckout) {
        return diMonthlyBookCheckoutCheckinRepo.save(monthlyBookCheckout);
    }

    //Get all di monthly-book-checkout
    public List<DIBookCheckoutCheckin> getAllBookCheckout() {
        return diMonthlyBookCheckoutCheckinRepo.findAll();
    }

    //Get di monthly-book-checkout with id
    public DIBookCheckoutCheckin getBookCheckoutById(long id) {
        Optional<DIBookCheckoutCheckin> bookCheckoutDB = diMonthlyBookCheckoutCheckinRepo.findById(id);
    ...
    //Update di monthly-book-checkout
    public DIBookCheckoutCheckin updateBookCheckout(DIBookCheckoutCheckin monthlyBookCheckout) {
        Optional<DIBookCheckoutCheckin> bookCheckoutDB = diMonthlyBookCheckoutCheckinRepo.findById(monthlyBookCheckout.getId());

        if (bookCheckoutDB.isPresent()) {
            DIBookCheckoutCheckin bookCheckoutUpdate = bookCheckoutDB.get();
            ...
            bookCheckoutUpdate.setRtrSchoolId(monthlyBookCheckout.getRtrSchoolId());
            bookCheckoutUpdate.setYearOfSupport(monthlyBookCheckout.getYearOfSupport());
            ...
            this.diMonthlyBookCheckoutCheckinRepo.save(bookCheckoutUpdate);
    ...
    //Delete di monthly-book-checkout
    public void deleteBookCheckout(long id) {
        Optional<DIBookCheckoutCheckin> bookCheckoutDB = this.diMonthlyBookCheckoutCheckinRepo.findById(id);

        if (bookCheckoutDB.isPresent()) {
            this.diMonthlyBookCheckoutCheckinRepo.delete(bookCheckoutDB.get());
        } else {
